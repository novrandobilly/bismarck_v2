...
import { useMenuItemMutations, type MenuItemFormData } from '@/pages/bismarck/menu/hooks/useMenuItemMutations'
...
function MenuItemChip({ item, onEdit }: { item: MenuItem; onEdit: (item: MenuItem) => void }) {
  return (
    <button
      type="button"
      onClick={() => onEdit(item)}
    ...
    >
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <span className="text-xs text-stone-300 group-hover:text-amber-500 transition-colors">✏️</span>
      </div>
    </button>
  )
}
    ...
    <div className="max-w-[1280px] mx-auto px-4 py-8 space-y-10">
            ...
            {activeItems.length > 0 && (
              <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {activeItems.map(item => <MenuItemChip key={item.id} item={item} onEdit={setEditTarget} />)}
                </div>
              </div>
            )}
            {archivedItems.length > 0 && (
              <div>
                ...
                <div className={cn('grid grid-cols-1 sm:grid-cols-2 gap-2 opacity-60')}>
                  {archivedItems.map(item => <MenuItemChip key={item.id} item={item} onEdit={setEditTarget} />)}
                </div>
              </div>
            )}
          </div>
      ...
      {isModalOpen && (
        <MenuItemFormModal
          item={editTarget}
          onSave={handleSave}
          onClose={() => { setEditTarget(undefined); setSaveError(null) }}
          isSaving={isSaving}
          saveError={saveError}
        />
      )}
